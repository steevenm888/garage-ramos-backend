...
const cache = fn => {
  let importations = new _joinTable2.default();
  let dependencies = new _joinTable2.default();
  let rememberedInput;
  let rememberedOutput;
  let queue = Promise.resolve((0, _castFilemap2.default)());

  const cachingTransform = _input => {
    queue = queue.then(async () => {
      ...
      const newImportations = new _joinTable2.default(); // <L: buildPath, R: importPath>
      const newDependencies = new _joinTable2.default(); // <L: buildPath, R: outputPath>

      // determine which input files have changed since last time
      const changedInput = oldInput ? (0, _diff2.default)(oldInput, input) : input;
      ...
      // build everything
      const results = await _bluebird2.default.props(filesToBuild.filter(content => content).map((content, buildPath) => {
        ...
        if (Buffer.isBuffer(content)) {
          ...
          return _bluebird2.default.resolve().then(() => fn.call(null, content, buildPath, include));
        }
      ...
      for (const buildPath of filesToBuild.keys()) {
        ...
        if (Buffer.isBuffer(result) || typeof result === 'string') {
          ...
          throw new TypeError("wire cache: Callback's return value is of invalid type " + `(${typeof result}) when building "${buildPath}"`);
        }
        ...
        for (let outputPath of outputPaths) {
          ...
          // (because if we were to just pick one, results might be undeterministic)
          {
            ...
            if (otherInputFile) {
              throw new Error(`wire cache: When building "${buildPath}"` + `the fn tried to output to "${outputPath}", but this has already ` + `been output by "${otherInputFile}"`);
            }
          ...
          if (typeof content === 'string') {
            ...
            throw new TypeError(`wire cache: Expected value for output file "${outputPath}" ` + `to be string or buffer; got ${typeof content}.`);
          }

          // make sure the path is normal (should be relative, with no "./" or "../")
          if (_path2.default.isAbsolute(outputPath)) {
            throw new Error(`wire cache: Expected a relative path, got: ${outputPath}`);
          }
          outputPath = _path2.default.normalize(outputPath);

          (0, _invariant2.default)(typeof content === 'object', "flow thinks this might be a number, and asserting Buffer.isBuffer doesn't seem to work");
      ...
      // finalise output
      const output = (0, _castFilemap2.default)(outputWrites);
