...
const cache = (fn: GranularBuildFunction): AsyncTransform => {
  ...
  const cachingTransform = _input => {
    queue = queue.then(async () => {
      ...
      const results: {
      ...
      } = await Bluebird.props(filesToBuild.filter(content => content).map((content, buildPath) => {
        ...
        if (Buffer.isBuffer(content)) {
          ...
          return Bluebird.resolve().then(() => fn.call(null, content, buildPath, include));
        }
      ...
      for (const buildPath of filesToBuild.keys()) {
        ...
        for (let outputPath of outputPaths) {
          ...
          invariant(typeof content === 'object', "flow thinks this might be a number, and asserting Buffer.isBuffer doesn't seem to work");
