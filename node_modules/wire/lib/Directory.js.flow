...
async function reprime(dir) {
  ...
  let totalSize = 0;
  const load = dirName => Bluebird.map(readdir(dirName), async _name => {
    ...
    if (stat.isFile()) {
      ...
      if (totalSize > dir._limit) {
        throw new Error(`Contents of directory exceed ${prettyBytes(dir._limit)} limit: ${dir._absolutePath}`);
      }
...
queueableMethods = {
  ...
  async write(incomingFiles: Filemap): Promise<Filemap> {
    ...
    for (const deletion of deletions) {
      await pruneEmptyAncestors(path.resolve(dir._absolutePath, deletion), dir._absolutePath);
    }
  ...
  async watch(subscriber: (: Filemap) => any, options?: { [: string]: any }): Promise<void> {
    const dir = this;
